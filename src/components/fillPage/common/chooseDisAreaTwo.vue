  ...
  <div>
    <section id="post" :class="[{ slip: showBox }, sidebar, location]">
      <ul id="post-city" class="first-list">
        ...
        <!--<li v-for="(item, index) in postCity"  :class="{selected:selCityEssCode == item.ESS_CITY_CODE}" :data-province-name="item.PROVINCE_NAME" :data-code="item.CITY_CODE" :data-ess-code="item.ESS_CITY_CODE" @click="selCity(item)">{{item.CITY_NAME}}</li>-->
      </ul>
      <ul id="post-district" class="second-list">
        ...
        <!--<li v-for="(item, index) in postDistrict"  :class="{selected:selDistrictCode == item.DISTRICT_CODE}" :data-code="item.DISTRICT_CODE" @click="selDistrict(item)">{{item.DISTRICT_NAME}}</li>-->
      </ul>
    </section>
