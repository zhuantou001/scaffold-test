      ...
      <el-col :span="18">
				<div class="img-list">
					<draggable v-model="transferArr" :move="getdata" @update="datadragEnd" @start="drag=true" @end="drag=false" :options="{animation: 200,handle:'.dragDiv'}" @dragover="allowDrop">
						<transition-group>
							<div class="img-content dragDiv" title='拖动排序'  :index="key" v-for="(item,key) in absoluteTopImgUrls" :key="key">
								<img :src="item.url">
								<!-- 放大icon -->
								<div class="layer">
...
export default{
	...
	data(){
      ...
      page_id: '',
			...
			enlargeImage: '',//放大图片地址
			params: {
				action: Config.api_url + ROUTES.headImgUpload,
				// action: 'https://demo.mall.10010.com:8108/scaffold-app' + ROUTES.headImgUpload,
				data: {}
			},
			page_alias: ''
		}
	...
	mounted () {
		// this.topImgFileList = this.imgList;
		this.transferArr = this.topImgFileList;
	},
	watch: {
		topImgShow: function () {
	...
	methods: {
		...
		datadragEnd (evt) {
			this.topImgFileList = this.transferArr;
			this.$store.commit('newTopImgFileList', this.topImgFileList);
		},
		allowDrop (evt) {
			evt.preventDefault();
		},
		...
		handleAvatarSuccess(res,file){//上传附件
			...
			if (res.resultCode === '0000') {
				...
				fileData.image_url = res.data.headImg;
				this.topImgFileList.push(fileData);
			} else {
				this.failMsg('上传失败');
			}
		},
		...
		handleRemove (file, fileList) {
			...
			for (i in this.topImgFileList) {
				if (file.name === this.topImgFileList[i].name) {
					this.$confirm('是否删除此附件？','提示',{
						...
						this.$ajax({
						method: 'get', //
						url: Config.api_url + ROUTES.deleteDetailImg + '?imgPath=' + this.topImgFileList[i].image_url + '&page_alias=' + this.page_alias,
						}).then((res)=>{
							if (res.data.data.ret_code === '0000') {
								self.successMsg("删除成功！");
							}
							self.topImgFileList.splice(i, 1);
							self.$store.commit('newTopImgFileList', this.topImgFileList);
							// let data = '';
							// if (localStorage.getItem('projectData')) {
							// 	data = JSON.parse(localStorage.getItem('projectData'));
							// }
    ...
    getComponentsItems () {
      return this.$store.state.app.getComponentsItems;
		...
		topImgFileList: {
			...
			set (value) {
				this.$store.commit('newTopImgFileList', value);
			}
		},
		// 绝对路径数组
		absoluteTopImgUrls () {
			...
			for (let i = 0; i < this.topImgFileList.length; i++) {
				let obj = {};
				obj.url = Config.img_prev_url + this.topImgFileList[i].image_url;
				// obj.url = 'https://demo.mall.10010.com:8108/scaffold-img' + this.topImgFileList[i].image_url;
				obj.name = this.topImgFileList[i].name;
				arr.push(obj);
			}
			return arr;
		}
